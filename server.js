...
// 보조 모듈 로드 (안내문 필수 포함 대상)
const requestValidation = require('./requestValidation');
const analysisResultStore = require('./analysisResultStore');
...
app.post('/api/transactions', async (req, res) => {
  ...
  if (!userId) return res.status(400).json({ success: false, error: 'userId는 필수 입력 항목입니다.' });
...
// 5. 위험 탐지 내역 조회 API (userId 쿼리 파라미터 지원 추가)
...
// 6. 전체/유저별 거래 목록 조회 API (userId 쿼리 파라미터 지원 추가)
app.get('/api/transactions', async (req, res) => {
  const { userId } = req.query;
  try {
    let queryStr = 'SELECT transaction_id, user_id, amount, merchant_name, merchant_category, transaction_datetime, transaction_status FROM transactions';
    const queryParams = [];
    ...
    queryStr += ' ORDER BY transaction_datetime ASC';

    const result = await pool.query(queryStr, queryParams);
    const rawTransactions = result.rows;

    const formattedTxList = rawTransactions.map(r => ({
      transaction_id: String(r.transaction_id),
      user_id: String(r.user_id),
      amount: Number(r.amount),
      merchant_name: String(r.merchant_name || '기타가맹점').trim(),
      merchant_category: String(r.merchant_category || '기타').trim(),
      transaction_datetime: new Date(r.transaction_datetime).toISOString(),
      transaction_status: r.transaction_status || 'APPROVED',
      transaction_type: 'CARD'
    }));

    const userHistoryMap = {};
    formattedTxList.forEach(tx => {
      if (!userHistoryMap[tx.user_id]) {
        userHistoryMap[tx.user_id] = [];
      }
      userHistoryMap[tx.user_id].push(tx);
    });

    const processedTransactions = formattedTxList.map(tx => {
      const userTxs = userHistoryMap[tx.user_id] || [];
      
      const pastHistory = userTxs.filter(pastTx => 
        pastTx.transaction_id !== tx.transaction_id &&
        new Date(pastTx.transaction_datetime).getTime() < new Date(tx.transaction_datetime).getTime()
      );
    ...
    console.error('❌ GET /api/transactions 처리 중 예외 발생:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});

// 7. [신규 추가] 특정 사용자의 누적 위험도(currentUserRisk) 조회 API
app.get('/api/users/:userId/risk', async (req, res) => {
  const { userId } = req.params;
  try {
    const result = await pool.query(
      'SELECT transaction_id, user_id, amount, merchant_name, merchant_category, transaction_datetime, transaction_status FROM transactions WHERE user_id = $1 ORDER BY transaction_datetime ASC',
      [userId]
    );
    ...
    const formattedTxList = result.rows.map(r => ({
      transaction_id: String(r.transaction_id),
      user_id: String(r.user_id),
      amount: Number(r.amount),
      merchant_name: String(r.merchant_name || '기타가맹점').trim(),
    ...
    const latestTx = formattedTxList[formattedTxList.length - 1];
    const pastHistory = formattedTxList.slice(0, -1);

    const latestAnalysis = analyzeTransactionRisk(
      latestTx,
      pastHistory,
      globalReferenceScores
    );

    const combinedScore = latestAnalysis.risk?.combinedScore || 0;
    const level = latestAnalysis.risk?.level || 'LOW';
    const activeRiskFactors = latestAnalysis.rule?.detectedRules || [];
    ...
    console.error('❌ GET /api/users/:userId/risk 처리 중 예외 발생:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});

// 8. [신규 추가] 단일 거래 상세 FDS 분석 조회 API
app.get('/api/transactions/:transactionId/analysis', async (req, res) => {
  const { transactionId } = req.params;
  try {
    const txQuery = await pool.query(
      'SELECT transaction_id, user_id, amount, merchant_name, merchant_category, transaction_datetime, transaction_status FROM transactions WHERE transaction_id = $1',
      [transactionId]
    );

    if (txQuery.rows.length === 0) {
      return res.status(404).json({ success: false, error: '해당 거래를 찾을 수 없습니다.' });
    }

    const targetTx = txQuery.rows[0];
    const formattedTarget = {
      transaction_id: String(targetTx.transaction_id),
      user_id: String(targetTx.user_id),
      amount: Number(targetTx.amount),
      merchant_name: String(targetTx.merchant_name || '기타가맹점').trim(),
      merchant_category: String(targetTx.merchant_category || '기타').trim(),
      transaction_datetime: new Date(targetTx.transaction_datetime).toISOString(),
      transaction_status: targetTx.transaction_status || 'APPROVED',
      transaction_type: 'CARD'
    };
    ...
    const historyQuery = await pool.query(
      `SELECT transaction_id, user_id, amount, merchant_name, merchant_category, transaction_datetime, transaction_status
       ...
       WHERE user_id = $1 
         ...
         AND transaction_id != $3
       ORDER BY transaction_datetime ASC`,
      [formattedTarget.user_id, targetTx.transaction_datetime, targetTx.transaction_id]
    );

    const formattedHistory = historyQuery.rows.map(r => ({
      transaction_id: String(r.transaction_id),
      user_id: String(r.user_id),
      amount: Number(r.amount),
      merchant_name: String(r.merchant_name || '기타가맹점').trim(),
    ...
    res.json({
      success: true,
      transactionId,
      transactionRisk: analysisResult
    });
  } catch (err) {
    console.error('❌ GET /api/transactions/:transactionId/analysis 처리 중 예외 발생:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});
