...
// 6. 전체 거래 목록 조회 API (수정됨: 메모리 최적화 및 Data Leakage 방지 FDS 분석 포함)
app.get('/api/transactions', async (req, res) => {
  try {
    // 1. 전체 거래 목록을 시간 오름차순(ASC)으로 1회만 DB 조회 (N+1 문제 방지)
    const result = await pool.query(
      'SELECT transaction_id, user_id, amount, merchant_name, merchant_category, transaction_datetime, transaction_status FROM transactions ORDER BY transaction_datetime ASC'
    );
    ...
    // 3. 각 거래별 이전 시점 history만 필터링하여 analyzeTransactionRisk 호출
    const processedTransactions = formattedTxList.map(tx => {
      const userTxs = userHistoryMap[tx.user_id] || [];
      
      // 현재 거래보다 명확히 이전 시점 거래만 포함 (Data Leakage 차단)
