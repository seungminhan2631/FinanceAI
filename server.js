...
async function initAiEngine() {
  try {
    ...
    // 요구사항 3번: Isolation Forest 모델 학습 및 동일 run에서 anomalyScore 참조 (0.5 fallback 제거)
...
app.post("/api/transactions", async (req, res) => {
  ...
  try {
    // 요구사항 5번: 신규 거래 DB 저장 (transaction_status = 'APPROVED' 명시)
    ...
    // 요구사항 4번: 현재 거래 시각 이전의 과거 history만 조회 (현재 거래 완전 제외)
    ...
    // 요구사항 13번: finalRiskAnalysisService 표준 결과 응답
...
// 요구사항 7번: DB/AI 동기화 완료 후 순차 구동 (startServer)
